...
export default function ParticipantDetail() {
  ...
  const events = useMemo(() => (validations.data ?? [])
    .filter((validation) => validation.status === "accepted" && validation.expand?.challenge?.scope === "individual")
    .map((validation) => ({
      team: validation.user,
      points_awarded: validation.points_awarded ?? 0,
  ...
  const done = new Set(list.filter((validation) => validation.status === "accepted").map((validation) => validation.challenge));
  ...
  return (
    <div className="mx-auto flex w-full max-w-3xl flex-col gap-4 px-4 py-4 md:gap-6 md:py-6">
      ...
      <Error message={getFirstErrorMessage(scores.error ?? teams.error ?? validations.error ?? challenges.error ?? wei.error)} />
      ...
      <section className="grid grid-cols-3 gap-4 rounded-md border border-border bg-card p-4 text-card-foreground shadow-sm sm:p-6">
        ...
        <Stat label="Défis validés" value={myScore ? String(myScore.validations_count ?? 0) : String(done.size)} />
      ...
      <section className="w-full rounded-md border border-border bg-card p-4 text-card-foreground shadow-sm sm:p-6">
        ...
        {list.length === 0 ? (
          <p className="mt-2 text-sm text-muted-foreground">Aucune preuve envoyée pour l'instant.</p>
        ) : (
      ...
      <section className="w-full rounded-md border border-border bg-card p-4 text-card-foreground shadow-sm sm:p-6">
        ...
        {fullHistory ? (
          ...
          <p className="mt-2 text-xs text-muted-foreground">
            Seules les validations rendues publiques sont visibles ici.
          </p>
        )}
